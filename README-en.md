...
* **Encapsulation and Rich Domain Model:** The main persistence entity restricts the mutability of its primary key (PK) and exposes behavioral methods to protect its internal state, laying the foundation for Domain-Driven Design (DDD) and complying with Clean Code principles.
* **Validation Perimeter and Standardized Errors:** Centralization and sanitization of Payloads through Jakarta Validation (@Valid). Failures are caught by a global interceptor (@ControllerAdvice) that dynamically resolves internationalization (i18n) and generates structured JSON responses.
