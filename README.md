...
* **Encapsulación y Modelo de Dominio Enriquecido:** La entidad principal de persistencia restringe la mutabilidad de su clave primaria (PK) y expone métodos de comportamiento para proteger su estado interno, sentando las bases para un Diseño Orientado al Dominio (DDD) y cumpliendo los principios Clean Code.
* **Perímetro de Validación y Errores Homologados:** Centralización y saneamiento de Payloads a través de Jakarta Validation (`@Valid`). Los fallos son capturados por un interceptor global (`@ControllerAdvice`) que resuelve la internacionalización (i18n) de forma dinámica y genera respuestas JSON y estructuradas.
